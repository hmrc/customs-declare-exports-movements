...
import uk.gov.hmrc.exports.movements.migrations.changelogs.movementNotifications.RemoveAnyIleQueryResponseISpec._

class RemoveAnyIleQueryResponseISpec extends ChangeLogsBaseISpec {

  private val mongoDatabase: MongoDatabase = {
    val mongoURI = mongoConfiguration.get[String]("mongodb.uri").replace("sslEnabled", "ssl")
    MongoClients.create(mongoURI).getDatabase(TestMongoDB.DatabaseName)
  }

  private val collection = mongoDatabase.getCollection("movementNotifications")

  private val changeLog = new RemoveAnyIleQueryResponse
  ...
  "RemoveAnyIleQueryResponse" should {

    "remove any IleQueryResponse Notification from the collection" in {
      val document = Document.parse(ileQueryResponse)
      assert(Option(collection.insertOne(document).getInsertedId).nonEmpty)

      changeLog.migrationFunction(mongoDatabase)

      assert(Option(collection.find().first).isEmpty)
    }

    "not remove any non-IleQueryResponse Notification from the collection" in {
      val document = Document.parse(movementTotalsResponse)
      assert(Option(collection.insertOne(document).getInsertedId).nonEmpty)

      changeLog.migrationFunction(mongoDatabase)

      assert(Option(collection.find().first).nonEmpty)
    }
  }
}

object RemoveAnyIleQueryResponseISpec {
