...
class ResponseValidatorSpec extends AnyWordSpec with Matchers {
  ...
  private val XSDFilePath = "conf/schemas/exports/inventoryLinkingResponseExternal.xsd"

  private val DefaultConfiguration = Configuration(
    ConfigFactory
      .parseString(s"""
        |mongodb.uri="mongodb://localhost:27017/customs-declare-exports-movements"
        |microservice.services.customs-inventory-linking-exports.schema-file-path=$XSDFilePath
        |""".stripMargin)
  )

  private def appConfig(conf: Configuration = DefaultConfiguration): AppConfig =
    new AppConfig(conf, new ServicesConfig(conf), "customs-declare-exports-movements")
  ...
  "ResponseValidator on validate" should {
    ...
    "return Success" when {
      ...
      def testSuccessScenario(input: NodeSeq): Unit = {
        ...
        assert(validator.validate(input).isRight)
      }
    }
    ...
    "return Failure" when {
      ...
      def testFailureScenario(input: NodeSeq): Unit = {
        ...
        assert(validator.validate(input).isLeft)
