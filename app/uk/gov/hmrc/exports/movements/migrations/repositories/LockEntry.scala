...
object LockEntry {
  ...
  def apply(document: Document): LockEntry =
    LockEntry(document.getString(KeyField), document.getString(StatusField), document.getString(OwnerField), document.getDate(ExpiresAtField))
}
