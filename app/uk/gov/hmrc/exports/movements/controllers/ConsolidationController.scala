...
class ConsolidationController @Inject() (consolidationService: SubmissionService, cc: ControllerComponents)(
  ...
  val submitConsolidation: Action[ConsolidationRequest] = Action.async(parse.json[ConsolidationRequest]) { implicit request =>
    consolidationService.submit(request.body).map(Accepted(_))
  }
}
